...
class Controller
{
    ...
    {
        // Nếu không truyền $resp, dùng $this->resp
        $response = $resp ?? $this->resp;
        // Đảm bảo $response luôn là object hợp lệ
        if (!is_object($response)) {
            $response = (object) ['result' => 0, 'msg' => 'Response not set'];
        }
        // In JSON và thoát
        echo json_encode($response);
        if (!defined('PHPUNIT_TEST')) {
            exit; // Chỉ thoát trong môi trường thực, không thoát trong test
        }
    }
}
